      ...
      <div className="container max-w-4xl mb-96">
        <div className="flex flex-col items-center space-y-8 text-center">
          ...
          <h1 className="text-4xl sm:text-6xl font-bold tracking-tighter">
            <span style={{color: '#da7756'}} className="transition-colors duration-300">
              Jason
            </span>
          </h1>
          
          <h2 className={`text-xl sm:text-2xl font-light ${isDark ? 'text-[#f5f2e8]/90' : 'text-[#3d3929]/90'} transition-colors duration-300`}>
            Full Stack Developer & UI/UX Enthusiast
          </h2>
          
          <p className={`max-w-2xl ${isDark ? 'text-[#f5f2e8]/70' : 'text-[#3d3929]/80'} text-lg h-[56px] transition-colors duration-300`}>
            <span>{text}</span>
            <span className={`inline-block w-2 h-5 bg-claude-salmon ml-1 ${isTyping ? 'animate-pulse' : 'opacity-0'} transition-colors duration-300`}></span>
            <br />
          ...
          <div className="flex gap-4 mt-8">
            <a
              href="#projects"
              style={{backgroundColor: '#bd5d3a'}}
              className="px-6 py-3 hover:opacity-90 rounded-full font-medium text-white transition-colors duration-300"
            >
              View My Work
            </a>
            <a
              href="#contact"
              style={{borderColor: '#bd5d3a'}}
              className={`px-6 py-3 border-2 rounded-full font-medium ${isDark ? 'text-[#f5f2e8]' : 'text-[#3d3929]'} hover:bg-claude-button/10 transition-colors duration-300`}
            >
              Contact Me
            </a>
          </div>
          ...
          <div className="flex gap-6 mt-8">
            <a href="https://github.com" target="_blank" rel="noopener noreferrer" className={`${isDark ? 'text-[#f5f2e8]' : 'text-[#3d3929]'} hover:opacity-75 transition-colors duration-300`}>
            ...
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className={`${isDark ? 'text-[#f5f2e8]' : 'text-[#3d3929]'} hover:opacity-75 transition-colors duration-300`}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </a>
            <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className={`${isDark ? 'text-[#f5f2e8]' : 'text-[#3d3929]'} hover:opacity-75 transition-colors duration-300`}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
